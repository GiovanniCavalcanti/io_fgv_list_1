...
Derive the share of each choice in each market.
...
$$
Where $V_k$ is the vector of observed characteristics and price. If we divide each probability, whihc is the observed market share, the denominator cancels out, yielding
$$
...
The folowing code fits the two requested models 
```{r}
# Step 1: Create the log difference between product shares and the outside option

# Filter to get the outside share for each market
outside_share <- dataset %>%
  filter(product == "0") %>%
  select(market, share) %>%
  rename(outside_share = share)

# Join with the original dataset to add the outside share to each product row
dataset_with_outside <- dataset %>%
  left_join(outside_share, by = "market") %>%
  filter(product != "0") %>%  # Exclude the outside good itself
  mutate(log_diff_share = log(share) - log(outside_share))

# Step 2: Estimate the model using OLS with product characteristics (x1 to x4) and price

# Fit the OLS model
ols_model <- lm(log_diff_share ~ x1 + x2 + x3 + x4 + price, data = dataset_with_outside)

# Step 3: Define IV model with instrumental variables (IV1-IV6) for endogenous variable (price)
iv_model <- ivreg(log_diff_share ~ x1 + x2 + x3 + x4 + price | . - price + iv1 + iv2 + iv3 + iv4 + iv5 + iv6, 
...
stargazer(ols_model, iv_model, type = "text", title = "Comparison of OLS and IV Model Results",
          column.labels = c("OLS Model", "IV Model"),
          covariate.labels = c("Characteristic X1", "Characteristic X2", "Characteristic X3", "Characteristic X4"),
          dep.var.labels = "Log Difference in Share (Product vs. Outside)",
          out.header = FALSE,
          model.names = FALSE)
```
Controlling for the instruments given yields an $\alpha$ estimand with higher magnitude, this is expected as the endogeneity problem would lower our price coefficient results.


1.5 Based on the IV parameters estimated above compute own- and cross-price elasticities for each good
in market one. Discuss the results
...
print(elasticity_matrix[1:5,1:5])
```
